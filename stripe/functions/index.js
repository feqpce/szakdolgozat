...
exports.addProductToCart = functions.https.onCall(async (data, context) => {
	const product = (await admin.firestore().collection('products').doc(data).get()).data();
	if (!product)
		return;
	return product;
});
...
exports.checkout = functions.https.onCall(async (data, context) => {
	const uid = context.auth.uid;
	const method = (await admin.firestore()
		.collection('stripe_customers')
...
function zeroDecimalCurrency(amount, currency) {
  let numberFormat = new Intl.NumberFormat(['hu-HU'], {
    style: 'currency',
    currency: currency,
